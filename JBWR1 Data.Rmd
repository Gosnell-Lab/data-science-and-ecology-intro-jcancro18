...
```{r}
head("~/Downloads/JBWR1.csv", stringsAsFactors = T)
```
...
P.Growth_Dilution_Relationship <- ggplot(JBWR1, aes(x=Dilution_Factor, y = Net_Growth_Rate)) +
  ...
  geom_errorbar(aes(ymin=Net_Growth_Rate-Std.Dev, ymax=Net_Growth_Rate+Std.Dev
), width=.01)
```
...
```{r}
P.Growth_Dilution_Relationship + ggtitle("Phytoplankton Growth Rate") +
  xlab("Dilution Factor") + ylab(bquote('Net Growth Rate '(µ^-1)))
```
...
```{r}
JBWR2 <- read.csv("~/Downloads/JBWR2.csv", stringsAsFactors = T)
#need to use stringsAsFactors to make characters read in as factors
```
...
```{r}
head("~/Downloads/JBWR2.csv", stringsAsFactors = T)
```
...
```{r}
P.Growth_Dilution_Relationship <- ggplot(JBWR2, aes(x=Dilution_Factor, y = Net_Growth_Rate)) +
  geom_point()+
  geom_smooth(method = "lm", se = FALSE)+
  geom_errorbar(aes(ymin=Net_Growth_Rate-Std.Dev, ymax=Net_Growth_Rate+Std.Dev
), width=.01)
...
### PART 3 (<5 um)
```{r}
JBWR3 <- read.csv("~/Downloads/JBWR3.csv", stringsAsFactors = T)
#need to use stringsAsFactors to make characters read in as factors
```
...
```{r}
head("~/Downloads/JBWR3.csv", stringsAsFactors = T)
```
...
```{r}
P.Growth_Dilution_Relationship <- ggplot(JBWR3, aes(x=Dilution_Factor, y = Net_Growth_Rate)) +
  stat_poly_line() +
  stat_poly_eq() +
  geom_point()+
  geom_smooth(method = "lm", se = FALSE)+
